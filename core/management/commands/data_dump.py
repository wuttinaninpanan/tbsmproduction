"""Snapshot the current database into ``core/fixtures/master_seed.json``.
...
``manage.py loaddata`` directly, but ``manage.py data_load`` wraps that
with a "delete-then-load" step.

Usage:
    python manage.py data_dump                       # default output path
    python manage.py data_dump --output other.json   # custom path
"""
...
DEFAULT_OUTPUT = Path("core/fixtures/master_seed.json")
