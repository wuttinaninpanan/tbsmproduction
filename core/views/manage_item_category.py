...
def download_manage_item_category_import_template(request):
	"""Download a template for importing item categories."""
	headers = ["name", "description"]
	rows = [["(ไม่ระบุ)", ""], ["Raw Material", ""]]
	if openpyxl is None:
		return HttpResponse(
	...
	response["Content-Disposition"] = 'attachment; filename="manage_item_category_import_template.xlsx"'
	wb.save(response)
	return response


@method_decorator(staff_required, name="dispatch")
class ManageItemCategoryViews(TemplateView):
	template_name = "manage_item_category.html"

	def get(self, request, *args, **kwargs):
		action = (request.GET.get("action") or "").strip().lower()
		if action == "download_template":
			return download_manage_item_category_import_template(request)
	...
	def post(self, request, *args, **kwargs):
		...
		if action == "import_master_data":
			...
			try:
				with transaction.atomic():
					for row in _parse_xlsx(upload):
						cat_name = _row_get_first(row, "name", "category", "category_name")
						desc = _row_get_first(row, "description", "desc")
						if not cat_name:
							skipped += 1
							continue
						if desc == "":
							desc = ""
						existing = ItemCategory.objects.filter(name__iexact=cat_name).first()
						if existing is None:
							...
							existing.save(update_fields=["name", "description", "updated_at"])
							updated += 1
			except Exception as e:
				log_event(
					request,
					action="item_category:import_master_data",
					status="failure",
					message="นำเข้า ItemCategory ไม่สำเร็จ",
			...
			transaction.on_commit(
				lambda: log_event(
					request,
					action="item_category:import_master_data",
					message="นำเข้า ItemCategory สำเร็จ",
					metadata={
		...
		if action == "bulk_delete_categories":
			...
			try:
				with transaction.atomic():
					for pk in ids:
						obj = ItemCategory.objects.filter(pk=pk).first()
						if obj is None:
							not_found += 1
							continue
						try:
				...
				log_event(
					request,
					action="item_category:bulk_delete",
					status="failure",
					message="ลบ ItemCategory แบบ bulk ไม่สำเร็จ",
			...
			transaction.on_commit(
				lambda: log_event(
					request,
					action="item_category:bulk_delete",
					message="ลบ ItemCategory แบบ bulk",
		...
		if action == "delete":
			...
			transaction.on_commit(
				lambda: log_event(
					request,
					action="item_category:delete",
					message="ลบ ItemCategory",
					metadata={"id": obj_id, "name": obj.name},
				),
			)
		...
		if action == "update":
			...
			transaction.on_commit(
				lambda: log_event(
					request,
					action="item_category:update",
					message="แก้ไข ItemCategory",
					metadata={"id": obj_id, "name": name},
				),
			)
		...
		try:
			...
			log_event(
				request,
				action="item_category:create",
				status="failure",
				message="เพิ่ม ItemCategory ไม่สำเร็จ",
		...
		transaction.on_commit(
			lambda: log_event(
				request,
				action="item_category:create",
				message="เพิ่ม ItemCategory",
				metadata={"id": str(obj.id), "name": obj.name},
			),
		)
