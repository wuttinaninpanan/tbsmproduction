(() => {
	const init = () => {
		...
		document.getElementById('addSubmit').addEventListener('click', () => {
			...
			displayNameField.value = (document.getElementById('addDisplayName').value || '').trim();
			actionForm.submit();
		});
		...
		document.querySelectorAll('[data-open-edit]').forEach(btn => {
			btn.addEventListener('click', () => {
				...
				editLabel.textContent = btn.dataset.name || '';
				openModal(editModal);
			});
		});
		document.getElementById('editSubmit').addEventListener('click', () => {
			...
			displayNameField.value = (document.getElementById('editDisplayName').value || '').trim();
			actionForm.submit();
		});
